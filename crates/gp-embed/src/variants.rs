use crate::download::is_installed;
use crate::hf::HfFile;
use crate::hf::install_local_id;
...
pub fn discover_onnx_variants(files: &[HfFile]) -> Vec<OnnxVariant> {
    ...
    for file in &paths {
        ...
        for other in files {
            let op = &other.path;
            if op.starts_with(stem)
                && (op.ends_with(".onnx_data") || op.contains(".onnx_data_"))
            {
                extra.push(op.clone());
                total += other.size;
            }
...
mod tests {
    ...
    fn groups_quant_and_full_precision() {
        ...
        assert!(labels.iter().any(|l| l.contains("model") && l.contains("MB")));
