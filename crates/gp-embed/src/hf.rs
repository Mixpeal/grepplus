...
impl HfClient {
    ...
    pub fn search_repos(&self, query: &str, limit: usize) -> Result<Vec<String>> {
        let url = format!("{HF_API}/models?search={}&limit={limit}", urlencoding(query));
        let resp = self
            .http
            .get(&url)
            .send()
    ...
    pub fn fetch_json<T: for<'de> Deserialize<'de>>(&self, repo: &str, revision: &str, file: &str) -> Result<T> {
...
pub fn classify_embedding_model(info: &HfModelInfo, force: bool) -> Result<EmbeddingCheck> {
    ...
    let pipeline = info.pipeline_tag.as_deref().unwrap_or_default().to_lowercase();
...
pub fn find_mirror_repos(client: &HfClient, repo: &str) -> Result<Vec<String>> {
    ...
    for query in [
        ...
        for id in client.search_repos(&query, 12)? {
            ...
            if lower.contains("onnx") && !lower.contains("gguf") {
