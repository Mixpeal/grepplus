...
use gp_core::query;
use gp_core::types::{RepoMeta, Route};
...
impl QueryFeatures {
    ...
    pub fn extract(query: &str, meta: &RepoMeta) -> Self {
        Self {
            ...
            has_camel_case: if query::has_camel_case(query.trim()) { 1.0 } else { 0.0 },
            has_regex: if query::has_regex_metacharacters(query) { 1.0 } else { 0.0 },
            has_nl_cue: if query::has_natural_language_cue(query) { 1.0 } else { 0.0 },
            is_literal: if query::is_literal_query(query) { 1.0 } else { 0.0 },
