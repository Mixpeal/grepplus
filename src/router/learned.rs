...
use crate::core::types::{RepoMeta, Route, RouteDecision};
use crate::router::features::{QueryFeatures, QueryFeatures as Qf};
...
pub fn train_from_traces(
    traces: &[crate::router::trace::RouteTrace],
) -> Result<LearnedRouterModel> {
    if traces.is_empty() {
...
pub fn save_model(path: &Path, model: &LearnedRouterModel) -> Result<()> {
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent).map_err(GpError::Io)?;
    }
    std::fs::write(path, serde_json::to_string_pretty(model)?).map_err(GpError::Io)?;
    Ok(())
}
...
mod tests {
    use super::*;
    use crate::router::trace::RouteTrace;
    ...
    fn trains_from_traces() {
        ...
        let symbol =
            model.predict_route(&QueryFeatures::extract("handleFoo", &RepoMeta::default()));
        ...
        assert!(matches!(
            nl,
            Route::Semantic | Route::Hybrid | Route::Prefocus
        ));
    }
}
