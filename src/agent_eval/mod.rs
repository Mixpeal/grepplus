...
pub fn run_agent_eval(
    ...
    warm_index: bool,
    yes_download: bool,
) -> Result<AgentEvalBundle, crate::core::error::GpError> {
    let _ = cfg;
    let queries = load_queries(suite)?;
    let addr: std::net::SocketAddr = serve_addr
        .parse()
        .map_err(|e| crate::core::error::GpError::Config(format!("invalid serve addr: {e}")))?;
        ...
        for delivery in deliveries {
            ...
            for q in &queries {
                ...
                let route = match retriever {
                    ...
                    "hybrid" => "hybrid",
                    other => {
                        return Err(crate::core::error::GpError::Config(format!(
                ...
                let body: SearchResponse = resp.json().map_err(|e| {
                    crate::core::error::GpError::Config(format!("decode search response: {e}"))
                })?;
...
fn maybe_spawn_server(
    ...
    warm_index: bool,
    yes_download: bool,
) -> Result<Option<std::process::Child>, crate::core::error::GpError> {
    if health_ok(addr) {
        return Ok(None);
    }
    ...
    let child = cmd
        .spawn()
        .map_err(|e| crate::core::error::GpError::Config(format!("spawn serve: {e}")))?;
...
fn wait_for_health(addr: std::net::SocketAddr) -> Result<(), crate::core::error::GpError> {
    for _ in 0..60 {
        if health_ok(addr) {
            return Ok(());
        }
    ...
    Err(crate::core::error::GpError::Config(
...
mod tests {
    ...
    fn path_matches_suffix() {
        assert!(path_matches("src/auth.rs", "auth.rs"));
        assert!(path_matches("auth.rs", "src/auth.rs"));
        assert!(!path_matches("other.rs", "auth.rs"));
    }

    #[test]
    fn format_agent_report_has_header() {
        let bundle = AgentEvalBundle { results: vec![] };
        let s = format_agent_report(&bundle);
        assert!(s.contains("retriever"));
    }
}
