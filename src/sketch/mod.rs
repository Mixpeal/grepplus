...
use crate::chunk::{chunk_file, Chunk, ChunkConfig};
use crate::core::error::{GpError, Result};
use crate::core::traits::PreFocus;
...
impl SketchBeam {
    pub fn build(roots: Vec<PathBuf>) -> Result<Self> {
        Self::build_with_options(roots, ChunkConfig::default(), &[])
    }
    ...
    pub fn build_with_options(
        ...
        for entry in walker.flatten() {
            ...
            let combined: String = file_chunks
                .iter()
                .map(|c| c.text.as_str())
                .collect::<Vec<_>>()
                .join("\n");
