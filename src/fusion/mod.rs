use crate::core::traits::Fusion;
use crate::core::types::{ChunkRef, RetrievalSource, ScoredChunk};
...
impl RrfFusion {
    ...
    pub fn fuse_lexical(
        &self,
        exact: Vec<ScoredChunk>,
        laser: Vec<ScoredChunk>,
    ) -> Vec<ScoredChunk> {
        let mut exact = exact;
...
fn pin_exact_hits(ranked: Vec<ScoredChunk>, exact_pins: &[ScoredChunk]) -> Vec<ScoredChunk> {
    ...
    for mut hit in ranked {
        if exact_pins
            .iter()
            .any(|pin| chunks_overlap(&hit.chunk, &pin.chunk))
        {
            let key = chunk_key(&hit.chunk);
