...
use crate::core::traits::Q4Code;
use crate::core::types::ChunkRef;
use crate::index::store::{path_hash, FileMeta, StoredChunkRecord};
use crate::index::temperature::{resolve_temperature, FileTemperature, TemperatureStats};
use crate::index::vectors::VectorCodec;
...
pub fn load_hot_chunks(
    ...
    if records.len() != codes.len() {
        return Err(GpError::Index(format!("chunk/vector mismatch for {rel}")));
    }
...
fn file_content_hash_for_repo(repo: &Path, rel: &str) -> String {
    crate::index::temperature::file_content_hash(&repo.join(rel)).unwrap_or_default()
}
