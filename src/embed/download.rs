...
pub fn model_dir(id: &str) -> PathBuf {
    crate::core::config::Config::models_dir().join(id)
}
...
pub fn list_installed() -> Result<Vec<String>> {
    let dir = crate::core::config::Config::models_dir();
    if !dir.exists() {
        return Ok(vec![]);
    }
    let mut out = Vec::new();
...
pub fn download_model(id: &str) -> Result<PathBuf> {
    let opts = crate::embed::pull::default_pull_opts(id);
    crate::embed::pull::install_model(id, &opts)
}
...
pub fn download_hf_path(
    ...
    let client = builder.build().map_err(|e| GpError::Model(e.to_string()))?;
