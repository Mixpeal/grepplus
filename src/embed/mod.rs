...
pub use download::{download_model, is_installed, list_installed, model_dir, remove_model};
pub use manifest::{ModelManifest, Pooling};
pub use onnx::OnnxEmbedder;
pub use picker::{
    ensure_model, interactive_pick, interactive_use_pick, list_installed_variants,
    print_models_list, set_active_model, EnsureOptions,
};
...
pub fn load_embedder(cfg: &Config, model_id: &str) -> Result<Arc<dyn Embedder>> {
    ...
    let embedder = OnnxEmbedder::load(&dir, cfg.embedder.dim, cfg.embedder.query_instruct.clone())?;
    Ok(Arc::new(embedder))
}
