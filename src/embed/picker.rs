...
use crate::embed::catalog::Catalog;
use crate::embed::download::{is_installed, list_installed, model_dir};
use crate::embed::hf::infer_base_id;
use crate::embed::manifest::ModelManifest;
use crate::embed::pull::{default_pull_opts, install_model, pull_model};
use console::style;
...
pub fn list_installed_variants() -> Result<Vec<InstalledVariant>> {
    ...
    for id in list_installed()? {
        ...
        let quant = manifest.quant.clone().unwrap_or_else(|| "unknown".into());
...
fn count_variants_for_base(base_id: &str, installed: &[InstalledVariant]) -> usize {
    installed.iter().filter(|v| v.base_id == base_id).count()
}
...
pub fn print_models_list(active: &str) -> Result<()> {
    ...
    for (base_id, group) in &groups {
        ...
        if is_output_tty() {
            println!("{}  {}", style(base_id).bold(), style(repo).dim());
        } else {
...
fn print_installed_variant_line(v: &InstalledVariant, active: &str) {
    ...
    if !is_output_tty() {
        println!("    {:<16} {:<26} {:>8}{tags}", v.quant, v.id, size);
        return;
    }
...
fn print_install_hint(catalog: &Catalog) -> Result<()> {
    let hint = if let Some(default) = catalog.default_id() {
        let size = catalog.get(default).map(|m| m.size_mb).unwrap_or(0);
        if size > 0 {
            format!(
            ...
            format!(
                "Run `grepplus models install` to browse recommended models (default: {default})."
            )
        }
    } else {
...
fn model_choices(catalog: &Catalog, installed: &[InstalledVariant]) -> Vec<ModelChoice> {
    ...
    for v in installed {
        if !catalog_ids.contains(v.base_id.as_str()) {
            other_by_base.entry(v.base_id.clone()).or_default().push(v);
        }
    }
...
fn activate_other_family(base_id: &str) -> Result<PickFlow> {
    let installed = list_installed_variants()?;
    let variants: Vec<_> = installed.iter().filter(|v| v.base_id == base_id).collect();
    if variants.is_empty() {
        return Err(GpError::Config(format!(
            "no installed variants for {base_id}"
        )));
    }
    if variants.len() == 1 {
    ...
    let labels: Vec<String> = variants.iter().map(|v| format_use_choice(v, "")).collect();
...
fn browse_hf_install() -> Result<PickFlow> {
    ...
    let id = path.file_name().and_then(|s| s.to_str()).unwrap_or(repo);
    set_active_model(id)?;
