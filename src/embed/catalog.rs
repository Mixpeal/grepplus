...
impl CatalogEntry {
    pub fn sha256_for(&self, file: &str) -> Option<&str> {
        ...
        } else if file == self.tokenizer_file && !self.sha256_tokenizer.is_empty() {
    ...
    pub fn format_kind(&self) -> crate::embed::manifest::ModelFormat {
        crate::embed::manifest::ModelFormat::parse(&self.format)
            .unwrap_or(crate::embed::manifest::ModelFormat::Onnx)
    }

    pub fn to_manifest(&self) -> crate::embed::manifest::ModelManifest {
        crate::embed::manifest::ModelManifest {
            id: self.id.clone(),
