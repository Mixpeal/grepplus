...
use crate::embed::manifest::{ModelManifest, Pooling};
use crate::embed::util::mrl_truncate_normalize;
use ndarray::Array2;
...
impl OnnxEmbedder {
    pub fn load(model_dir: &Path, dim: usize, query_instruct: String) -> Result<Self> {
        ...
        let session = crate::embed::ort_session::open_session(&model_dir.join(&cfg.model_file))?;
    ...
    fn run_session<'a>(
        ...
        let mut id_val =
            Some(Value::from_array(ids.clone()).map_err(|e| GpError::Model(e.to_string()))?);
        let mut mask_val =
            Some(Value::from_array(mask.clone()).map_err(|e| GpError::Model(e.to_string()))?);
        let mut type_val =
            Some(Value::from_array(type_ids.clone()).map_err(|e| GpError::Model(e.to_string()))?);
        let mut pos_val =
            Some(Value::from_array(positions.clone()).map_err(|e| GpError::Model(e.to_string()))?);
        ...
        for name in &self.input_names {
            let value = match feed_kind_for_input(name) {
                Some(FeedKind::InputIds) => id_val
                    .take()
                    .ok_or_else(|| GpError::Model("duplicate input_ids in ONNX graph".into()))?,
                ...
                Some(FeedKind::PositionIds) => pos_val
                    .take()
                    .ok_or_else(|| GpError::Model("duplicate position_ids in ONNX graph".into()))?,
                None => return Err(GpError::Model(format!("unsupported ONNX input `{name}`"))),
            };
...
mod onnx_live_tests {
    ...
    fn harrier_dir() -> Option<PathBuf> {
        let dir = crate::core::config::Config::models_dir().join("harrier-oss-v1-0.6b");
    ...
    fn bge_dir() -> Option<PathBuf> {
        let dir = crate::core::config::Config::models_dir().join("bge-small-en-v1.5");
        if dir.join("manifest.json").exists() {
            Some(dir)
        } else {
            None
    ...
    fn nomic_fp16_live_probe() {
        let dir =
            crate::core::config::Config::models_dir().join("nomic-embed-text-v1.5-model_fp16");
        if !dir.join("manifest.json").exists() {
