...
use crate::embed::catalog::{Catalog, CatalogEntry};
use crate::embed::download::{download_hf_path, is_installed, model_dir};
use crate::embed::hf::{
...
use crate::embed::progress::with_spinner;
use crate::embed::variants::{
...
pub fn install_model(id: &str, opts: &PullOptions) -> Result<PathBuf> {
    ...
    let entry = catalog
        ...
        .clone();
    install_onnx(&entry.repo, &entry.revision, &entry.id, Some(&entry), opts)
}
...
pub fn pull_model(opts: &PullOptions) -> Result<PathBuf> {
    ...
    match classify_embedding_model(&info, opts.force)? {
        ...
        EmbeddingCheck::Reject(msg) if !opts.force => {
            return Err(GpError::Model(format!("{msg} (use --force to override)")));
        }
        ...
        match pick_onnx_variant(
            ...
            OnnxVariantPick::ChangeExport => {
                return Err(GpError::Config(
                    "unexpected back in single-repo pull".into(),
                ))
            }
        }
    };
...
fn smoke_test(model_id: &str) -> Result<()> {
    ...
    let _ = crate::embed::load_embedder(&cfg, model_id)?;
    Ok(())
}
